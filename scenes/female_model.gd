...
func _on_item_taken(nam, count, item_path):
	if main_skeleton == "Female_palladium_armature" and game_params.story_vars.apata_trap_stage == PLDGameParams.TrapStages.ARMED: # Apply only to female model and only when the trap is still armed
		if nam == "statue_apata":
