...
func is_active():
	return game_params.story_vars.apata_trap_stage == PLDGameParams.TrapStages.ACTIVE
...
func activate():
	game_params.story_vars.apata_trap_stage = PLDGameParams.TrapStages.ACTIVE
...
func pause():
	game_params.story_vars.apata_trap_stage = PLDGameParams.TrapStages.PAUSED
	get_node("ceiling_armat000/AnimationPlayer").stop(false)
	get_node("AnimationPlayer").stop(false)
	ceiling_sound_stop()

func deactivate():
	game_params.story_vars.apata_trap_stage = PLDGameParams.TrapStages.DISABLED
	var speed = get_ceiling_speed()
...
func restore_state():
	if is_active():
		activate()
	elif game_params.story_vars.apata_trap_stage == PLDGameParams.TrapStages.PAUSED:
		activate()
...
func _on_AnimationPlayer_animation_finished(anim_name):
	if game_params.story_vars.apata_trap_stage == PLDGameParams.TrapStages.DISABLED:
		ceiling_sound_stop()
		if ( \
