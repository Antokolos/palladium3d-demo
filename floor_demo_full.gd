...
func _on_AreaApata_body_entered(body):
	var player = get_node(game_params.player_path)
	var target = get_node(game_params.companion_path)
	if game_params.apata_on_pedestal and body.is_in_group("party") and conversation_manager.conversation_is_not_finished(player, target, "ApataStatue"):
		conversation_manager.start_conversation(player, target, "ApataStatue")

func _on_AreaMuses_body_entered(body):
	var player = get_node(game_params.player_path)
	var target = get_node(game_params.companion_path)
	if game_params.apata_in_chest and body.is_in_group("party") and body.is_player() and conversation_manager.conversation_is_not_finished(player, target, "Muses"):
